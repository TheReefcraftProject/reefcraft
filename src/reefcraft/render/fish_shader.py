...
"""Fish shaped shader/material for particles."""
import pathlib
...
class FishPointsMaterial(PointsMaterial):
    """Custom material so we don't override normal PointsMaterial."""
    pass

@register_wgpu_render_function(Points, FishPointsMaterial)
class FishPointsShader(BaseShader):
    """Fish shaped shader for particles."""
    type = "render" #render pass shader, not compute

    def __init__(self, wobject: gfx.WorldObject) -> None:
        """Initialize fish shader."""
        super().__init__(wobject)

        material = cast("FishPointsMaterial", wobject.material)
    ...
    def get_code(self) -> str:
        """Define WGSL shader code for fish particles."""
        shader_path = pathlib.Path(__file__).parent/"fish.wgsl"
...
def make_test_point() -> gfx.Points:
    ...
    material = FishPointsMaterial(color = (1.0, 0.2, 0.8, 1.0), size = 1000)
