...
fn vs_main(in: VertexInput) -> Varyings {
    ...
    let shift: f32 = 1000.0;

    let pos_s_dx = pos_s + vec2<f32>(shift, 0.0);
    let pos_s_dy = pos_s + vec2<f32>(0.0, shift);

    let pos_n_dx = vec4<f32>((pos_s_dx / screen_factor - 1.0) * pos_n.w, pos_n.z, pos_n.w);
    let pos_n_dy = vec4<f32>((pos_s_dy / screen_factor - 1.0) * pos_n.w, pos_n.z, pos_n.w);

    let pos_w_dx = u_stdinfo.cam_transform_inv * u_stdinfo.projection_transform_inv * pos_n_dx;
    let pos_w_dy = u_stdinfo.cam_transform_inv * u_stdinfo.projection_transform_inv * pos_n_dy;

    // World units per *logical* pixel in X and Y directions
    let ratio_x = (1.0 / shift) * distance(pos_w.xyz, pos_w_dx.xyz);
    let ratio_y = (1.0 / shift) * distance(pos_w.xyz, pos_w_dy.xyz);
    ...
    $$ else
        ...
        $$ else
            ...
            let size_ratio = (1.0 / shift_factor) * 0.5 * (distance(pos_w.xyz, pos_w_dx.xyz) + distance(pos_w.xyz, pos_w_dy.xyz));
        $$ endif
    $$ endif
    ...
    //per-fish yaw (vertex mode) or material rotation
    $$ if rotation_mode == 'vertex'
        var yaw = load_s_rotations(node_index);
    $$ else
        var yaw = u_material.rotation;
    $$ endif

    //if your fish point "down" or "tail-first", try adding an offset: +0.5 * PI or -0.5 * PI
    yaw -= 0.5 * PI;

    //local world axes: right in XZ by yaw; up is global Y (keeps fish upright)
    let right = vec3<f32>(cos(yaw), 0.0, -sin(yaw));
    let forward = vec3<f32>(sin(yaw), 0.0,  cos(yaw));  // +Z when yaw=0

    //tilt the quad a bit upward so it’s not a flat "floor"
    let pitch: f32 = 1.5;
    let fwd_tilt = normalize(forward * cos(pitch) + vec3<f32>(0.0, 1.0, 0.0) * sin(pitch));

    //convert the quad's screen-space delta (logical pixels) to world-space offset
    let dx_world = the_delta_s.x * ratio_x;
    let dy_world = the_delta_s.y * ratio_y;

    //place this vertex in world space
    let world_p = pos_w.xyz + right * dx_world + fwd_tilt * dy_world;

    //project to clip space
    let the_pos_n = u_stdinfo.projection_transform * (u_stdinfo.cam_transform * vec4<f32>(world_p, 1.0));
...
fn fs_main(varyings: Varyings) -> FragmentOutput {
    ...
    //local quad coords in ~[-1, 1] (unit-radius circle)
    let uv = pointcoord_p / half_size_p;

    // Body (ellipse)
    let body_center = vec2<f32>( 0.12, 0.0);
    let d_body = sdEllipse(uv - body_center, vec2<f32>(0.40, 0.15));

    // Tail params (cartoony but clear)
    let half_w = 0.13;   // tail half-width
    let h      = 0.35;   // tail length (apex→base)

    // --- Rotate triangle so its APEX points toward +X (into the body) ---
    let ang = -0.5 * PI; // -90°

    // Choose a base location so the apex lands just inside the ellipse.
    // With our SDF, apex is at (0,h) BEFORE rotation, so AFTER -90° it’s at (+h, 0).
    // Therefore apex world pos ≈ tail_base + vec2(h, 0).
    var tail_base = vec2<f32>(body_center.x - (h + 0.06), 0.0);
    tail_base.x -= 0.3;

    // Rotate local coords and evaluate SDF
    let p_tail = rot2(ang) * (uv - tail_base);
    // Add a subtle arch at the base of the tail so it looks less flat/cartoonish
    let arch_amount = 0.04; // tweak 0.02..0.06 for gentler/stronger arch
    let d_tail = sdIsoTriArched(p_tail, half_w, h, arch_amount);

    // Soft union, small k so tail doesn’t get eaten
    let smooth_k = 0.2;
    var fish_d = opSmoothUnion(d_body, d_tail, smooth_k);

    // Mask
    let edge = 0.8 / half_size_p;
    let fill_mask = 1.0 - smoothstep(0.0, edge, fish_d);

    face_alpha = fill_mask;

    // --- Outer glow (in pixels) ---
    let glow_px      = 5.0;                 // thickness of the halo in screen pixels
    let w            = glow_px / half_size_p; // convert to uv units
    let d_out        = max(fish_d, 0.0);          // only outside the fish (d>0)
    let glow_alpha   = 1.0 - smoothstep(0.0, w, d_out);

    // optional shaping for a softer rim (comment out to keep linear-ish falloff)
    let glow_gamma   = 0.5;                  // >1.0 makes the rim tighter
    let glow_alpha_s = pow(glow_alpha, glow_gamma);
    ...
    // --- Apply outer glow before the discard ---
    // tint + strength controls
    let glow_tint     = the_color.rgb;       // or vec3<f32>(1.0, 0.9, 1.0)
    let glow_strength = 0.6;

    // premix new rgb
    let new_rgb = mix(the_color.rgb, glow_tint, glow_alpha_s * glow_strength);

    // ensure pixels outside the fish survive the discard
    let new_a = max(the_color.a, glow_alpha_s * glow_strength);

    // reassign the whole vec4 (WGSL-friendly)
    the_color = vec4<f32>(new_rgb, new_a);
...
fn sdEllipse(p: vec2<f32>, r: vec2<f32>) -> f32{
    //distance < 0 -> inside ellipse
    let k = p/r;
    return length(k) - 1.0;
}

// Isosceles triangle via 3 half-planes (robust, negative inside).
// Vertices: A(-half_w,0), B(half_w,0), C(0,h)
fn sdIsoTri(p: vec2<f32>, half_w: f32, h: f32) -> f32 {
    let A = vec2<f32>(-half_w, 0.0);
    let B = vec2<f32>( half_w, 0.0);
    let C = vec2<f32>(0.0, h);

    // Outward normals for each edge (ccw winding)
    let nAB = vec2<f32>(0.0, -1.0);                     // base, outward is down
    let eAC = normalize(C - A);
    let nAC = vec2<f32>(-eAC.y, eAC.x);                 // rotate 90° ccw
    let eBC = normalize(C - B);
    let nBC = vec2<f32>( eBC.y, -eBC.x);                // rotate 90° cw

    // Signed distances to edges (positive = outside)
    let d1 = dot(p - A, nAB);
    let d2 = dot(p - A, nAC);
    let d3 = dot(p - B, nBC);

    // For convex polygons, max of edge distances gives a signed distance field
    // that is <= 0 inside, > 0 outside, and 0 on the boundary.
    return max(max(d1, d2), d3);
}

// Isosceles triangle with a gently arched base.
// The base y=0 is bent upwards by a small amount near x=0 to avoid a perfectly straight base.
// 'arch' controls the peak offset at the center of the base in the same units as 'p'.
fn sdIsoTriArched(p: vec2<f32>, half_w: f32, h: f32, arch: f32) -> f32 {
    // Compute a parabolic offset across the base width, clamped to [-half_w, half_w]
    let px = abs(p.x) / max(half_w, 0.0001);
    let t = saturate(px);
    let arch_offset = arch * (1.0 - t * t);

    // Bend the base upward by subtracting the offset from y before evaluating the triangle SDF
    let p2 = vec2<f32>(p.x, p.y - arch_offset);
    return sdIsoTri(p2, half_w, h);
}

//smooth union to blend body and tail
