...
class ComputeLBM:
    ...
    def __init__(self, grid_shape, num_steps, fluid_speed, stl_filename, post_process_interval):
        ...
        self.stl_filename = os.path.join(os.path.dirname(__file__), '..', 'resources', 'stl', 'coral.stl')
    ...
    def get_coral_mesh_field(self):
        """
        Generate a 3D field for the coral mesh where each voxel is marked as part of the coral mesh.
        Returns a numpy array where 1 represents coral and 0 represents empty space.
        """
        # Create an empty 3D grid (mesh field) representing the simulation space
        coral_mesh_field = np.zeros(self.grid_shape, dtype=np.float32)

        # Loop through the mesh vertices and mark the grid cells that intersect with the coral mesh
        for i in range(self.coral_vertices.shape[0]):
            # Map the mesh vertices to grid coordinates (rounded to nearest grid index)
            ...
            if 0 <= ix < self.grid_shape[0] and 0 <= iy < self.grid_shape[1] and 0 <= iz < self.grid_shape[2]:
                coral_mesh_field[ix, iy, iz] = 1  # Mark as part of the coral mesh

        return coral_mesh_field
    
    def get_field_numpy(self) -> dict:
        """
        Returns velocity, density, fields as dictionary of numpy arrays.
        Returns keys:
            "density"
            "pressure"
            "velocity"
            "velocity_magnitude"
            "coral_mesh"  # Added key for coral mesh
        """
        # Create pre-allocated fields
        rho_field = self.grid.create_field(cardinality=1)  # 3D density field (1 channel)
        u_field = self.grid.create_field(cardinality=self.velocity_set.d)  # 3D velocity field (3 channels)
        ...
        # Convert coral mesh field to NumPy array
