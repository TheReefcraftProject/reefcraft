...
fn fs_main(varyings: Varyings) -> FragmentOutput {
    ...
    let x = pointcoord_p.x;
    let y = pointcoord_p.y;

    var new_x = x * (1.0 - abs(y) * 0.1);
    ...
    // ---- per-point flicker ----------------------------------
    const FLICKER_SPEED: f32 = 1.2;   // cycles per second
    const FLICKER_MIN:   f32 = 0.35;  // never go fully dark

    let seed   = hash11(f32(varyings.pick_idx) + 13.0);
    let phase  = u_material.rotation * 6.2831853 * FLICKER_SPEED + seed * 6.2831853;
    let twink  = 0.5 + 0.5 * sin(phase);                    // 0..1
    let gain   = mix(FLICKER_MIN, 1.0, twink);              // FLICKER_MIN..1
    // ---------------------------------------------------------

    // Determine final color and opacity
    if (the_color.a <= 0.0) { discard; }
    $$ if color_mode == 'debug'
        let out_color = vec4<f32>(srgb2physical(the_color.rgb) * gain, the_color.a);
    $$ else
        let out_color = vec4<f32>(srgb2physical(the_color.rgb) * gain, the_color.a * u_material.opacity);
    $$ endif
